...
//    mVertices.push_back({v3.x, v3.y, v3.z, v3.x, v3.y, v3.z, 0.5f + std::atan2(-v3.z, -v3.x)/2 * gsl::PI, 0.5f - std::asin(-v3.y) / gsl::PI});
   ...
   // v.set_st(0.5f + std::atan2(-v1.z, -v1.x)/2 * gsl::PI, 0.5f - std::asin(-v1.y) / gsl::PI);			// kan utelates
   v.set_st(0.f, 0.f);
   mVertices.push_back(v);
   v.set_xyz(v2.x, v2.y, v2.z);
   v.set_normal(v2.x, v2.y, v2.z);
   // v.set_st(0.5f + std::atan2(-v2.z, -v2.x)/2 * gsl::PI, 0.5f - std::asin(-v2.y) / gsl::PI);
   v.set_st(0.5f, 1.f);
   mVertices.push_back(v);
   v.set_xyz(v3.x, v3.y, v3.z);
   v.set_normal(v3.x, v3.y, v3.z);
   // v.set_st(0.5f + std::atan2(-v3.z, -v3.x)/2 * gsl::PI, 0.5f - std::asin(-v3.y) / gsl::PI);
   v.set_st(1.f, 0.f);
   mVertices.push_back(v);
